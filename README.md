...
本应用库为了解决上述问题，把各个环节的下载地址替换成了国内可加速访问的地址。本应用库使用的是 [GitHub Proxy](https://v6.gh-proxy.org/) 和 [GitHub Actions](https://github.com/features/actions) 。
...
```powershell
irm https://v6.gh-proxy.org/https://raw.githubusercontent.com/duzyn/scoop-cn/master/install.ps1 | iex
```
...
1. 运行以下命令添加本仓库
    ...
    ```powershell
    scoop bucket add scoop-cn https://v6.gh-proxy.org/https://github.com/duzyn/scoop-cn
    ```
...
运行以下命令设置新 GitHub 代理地址，下例中 `https://v6.gh-proxy.org` 是当前在使用的：

```powershell
scoop config scoop_repo https://v6.gh-proxy.org/https://github.com/ScoopInstaller/Scoop
git -C "$env:USERPROFILE\scoop\buckets\main" remote set-url origin https://v6.gh-proxy.org/https://github.com/ScoopInstaller/Main
git -C "$env:USERPROFILE\scoop\buckets\scoop-cn" remote set-url origin https://v6.gh-proxy.org/https://github.com/duzyn/scoop-cn
```
